	...
	socket.on('newPacket', function (packet) {
		...
		for (var player in players){
			if (player != packet["name"] && players[packet["name"]]){
				if ((Math.sqrt((players[player]["position"][0]-players[packet["name"]]["position"][0])**2 + (players[player]["position"][1]-players[packet["name"]]["position"][1])**2)) < (players[player]["size"] + players[packet["name"]]["size"])/2){
